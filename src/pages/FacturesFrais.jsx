...
export default function FacturesFrais({ user, onBack, onLogout }) {
  ...
  const loadCats = async () => {
    try { const d = await getInvCats(); setCats(Array.isArray(d) ? d : []); }
  ...
  const upload = async () => {
    ...
    try {
      ...
      setFile(null);
      setForm({description:'', montant:'', categorie:'', date_facture:''});
      setShowUpload(false);
      loadInvoices();
  ...
  const doDelCat    = async (id) => { await delInvCat(id); await loadCats(); toast$('Supprimée'); };
          ...
          <div className={styles.list}>
            ...
            {invoices.map(inv => (
              <div key={inv.id} className={styles.row}>
                <span className={styles.rowDate}>{fmt(inv.date_facture || inv.created_at)}</span>
                ...
                <span className={styles.rowDesc}>{inv.description || '—'}</span>
